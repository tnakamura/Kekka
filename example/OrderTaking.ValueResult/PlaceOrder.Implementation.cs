...
public delegate Task<ValueResult<CheckedAddress, AddressValidationError>> CheckAddressExists(UnvalidatedAddress address);
...
public delegate Task<ValueResult<ValidatedOrder, ValidationError>> ValidateOrder(
...
public delegate ValueResult<PricedOrder, PricingError> PriceOrder(
...
{
    public static ValueResult<CustomerInfo, ValidationError> ToCustomerInfo(this UnvalidatedCustomerInfo unvalidatedCustomerInfo)
    {
        var result =
    ...
    public static ValueResult<Address, ValidationError> ToAddress(CheckedAddress checkedAddress)
    {
        var result =
    ...
    /// </summary>
    public static Task<ValueResult<CheckedAddress, ValidationError>> ToCheckedAddress(CheckAddressExists checkAddress, UnvalidatedAddress address)
    {
        var result =
    ...
    public static ValueResult<OrderId, ValidationError> ToOrderId(this string orderId) =>
        OrderId.Create("OrderId", orderId).MapError(x => new ValidationError(x));

    /// <summary>
    /// Helper function for validateOrder
    /// </summary>
    public static ValueResult<OrderLineId, ValidationError> ToOrderLineId(this string orderId) =>
        OrderLineId.Create("OrderLineId", orderId).MapError(x => new ValidationError(x));

    /// Helper function for validateOrder
    public static ValueResult<ProductCode, ValidationError> ToProductCode(CheckProductCodeExists checkProductCodeExists, string productCode)
    {
        // create a ProductCode -> ValueResult<ProductCode,...> function
        // suitable for using in a pipeline
        ValueResult<ProductCode, ValidationError> CheckProduct(ProductCode productCode)
        {
            if (checkProductCodeExists(productCode))
            {
                return ValueResult.Ok<ProductCode, ValidationError>(productCode);
            }
            else
            {
                return ValueResult.Error<ProductCode, ValidationError>(
    ...
    /// Helper function for validateOrder
    /// </summary>
    public static ValueResult<OrderQuantity, ValidationError> ToOrderQuantity(ProductCode productCode, decimal quantity) =>
    ...
    /// Helper function for validateOrder
    /// </summary>
    public static ValueResult<ValidatedOrderLine, ValidationError> ToValidatedOrderLine(
...
{
    public static ValueResult<PricedOrderLine, PricingError> ToPricedOrderLine(GetProductPrice getProductPrice, ValidatedOrderLine validatedOrderLine)
    {
        var result =
