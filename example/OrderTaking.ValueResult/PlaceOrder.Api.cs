...
public static class Workflow
{
    ...
    internal static readonly Implementation.CheckAddressExists checkAddressExists =
        unvalidatedAddress =>
        {
            ...
            return ValueResult.Ok<CheckedAddress, AddressValidationError>(checkedAddress).AsTask();
        };
    ...
    public static HttpResponse WorkflowValueResultToHttpReponse(
        ValueResult<IList<PlaceOrderEvent>, PlaceOrderError> result)
    {
    ...
    public static readonly PlaceOrderApi PlaceOrderApi = async request =>
    {
        ...
        var asyncValueResult = await workflow(unvalidatedOrder);

        // now convert from the pure domain back to a HttpResponse
        return WorkflowValueResultToHttpReponse(asyncValueResult);
    };
}
